...
export const accountingSaveInvoice=createServerFn({method:"POST"}).middleware(auth).inputValidator((i:unknown)=>invoiceSchema.parse(i)).handler(async({data,context})=>{await assertAdmin(context.supabase,context.userId);const d=await db();const {items,...invoice}=data;const{data:id,error}=await d.rpc("save_accounting_invoice",{_invoice:invoice,_items:items});if(error)throw error;return{id};});
